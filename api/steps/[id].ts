...
export default async function handler(req: RequestLike, res: JsonResponse) {
  ...
  try {
    ...
    if (!id) {
      res.status(400).json({ error: 'Missing steps log id' })
      return
    }

    const result = await prisma.stepsLog.deleteMany({
      where: {
        id,
        userId: user.id,
      },
    ...
    if (result.count === 0) {
      res.status(404).json({ error: 'Steps entry not found' })
      return
    }
    ...
    console.error('[api/steps/[id]]', error)
    res.status(500).json({
